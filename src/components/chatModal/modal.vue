  ...
  <div v-show="show" class="modal-mask" >
    <div class="modal-container box">
      <div class="hang" @click="closeHandle">
        <img :src="closeImg" alt="关闭"   class="icon" />
      </div>
...
import DoubleVideo from '/@/components/chatRoom/components/videoBox/double.vue'
          ...
          const messageList = messageListSuperList.reverse().map(item => {
            ...
            return {
              ...
              fromUserInfo:{
                ...
                avatar: buildImgUrl(fromSysUser.avatar || "", baseUrl),
              },
...
onMounted( ()=> {
  ...
  // 初始化聊天室
  Promise.all([chatStore.initRoom(), chatStore.initFriendList({notSelf:1})]).then(()=> {
    chatStore.connect({
      reconnection:true,
              ...
              if (content) {
                ...
                const m = {
                  ...
                  fromUserInfo:{
                    uid: uid,
                    nickname: nickname,
                    avatar: buildImgUrl(avatar, baseUrl),
                  },
